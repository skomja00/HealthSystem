...
public class ValidationUtils {
    ...
    /* Convert "val" (String) to java.sql.DateTime and return the converted date. */
    public static java.sql.Date dateTimeConversion(String val) {

        if ((val == null) || (val.length() == 0)) {
            return null;
        }
        try {
            java.text.SimpleDateFormat dateformat = new java.text.SimpleDateFormat("MM/dd/yyyy h:mm a"); //please notice the capital M
            dateformat.setLenient(false);
            java.util.Date myDate = dateformat.parse(val);
            return new java.sql.Date(myDate.getTime());
            //return d.toString(); // debugging...
