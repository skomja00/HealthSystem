...
public class PrepStatement {
    ...
    */
    ...
    // Replace the position-th question mark with newDate.
    // If newDate is null, encode null into that question mark.
    public String setDateTime(int position, java.time.LocalDateTime newDate) {
        try {
            if (newDate == null) {
                ps.setNull(position, java.sql.Types.DATE);
                ...
                //TODO: setDateTime
                this.ps.setTimestamp(position, timestamp);
                //this.ps.setTimeStamp(position, newDate);
            }
            return ""; // no error
        } catch (Exception e) {
            String msg = " PrepStatement: Exception in setDateTime(). Sql is " + this.sql
                    + ", position: " + position + ". Error Msg: " + e.getMessage();
            System.out.println("====> " + msg);
            this.errorMsg += msg;
            return msg;
